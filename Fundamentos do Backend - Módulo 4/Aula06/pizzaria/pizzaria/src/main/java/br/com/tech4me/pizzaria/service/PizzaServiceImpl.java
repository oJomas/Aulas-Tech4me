...
public class PizzaServiceImpl implements PizzaService{
    ...
    public List<PizzaDTO> obterTodas() {
        return repositorio.findAll().stream().map(p -> new PizzaDTO(p.getId(), p.getSabor(), p.getIngredientes())).toList();
    }
    ...
    public Optional<PizzariaCompletoDTO> atualizarPorId(String id, PizzariaCompletoDTO dto) {
        ...
        if (pizza.isPresent()) {
            ...
            return Optional.of(new PizzariaCompletoDTO(p.getId(), p.getSabor(), p.getIngredientes(), p.getTamanho(), p.getValor()));
        }
